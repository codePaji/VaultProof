...
export default function VotePage() {
  ...
  const [voterSecretHex, setVoterSecretHex] = useState<string>(() => {
    let secret = localStorage.getItem('vaultproof_voter_secret');
    if (!secret) {
      const randomBytes = new Uint8Array(32);
      crypto.getRandomValues(randomBytes);
      secret = Array.from(randomBytes).map(b => b.toString(16).padStart(2, '0')).join('');
      localStorage.setItem('vaultproof_voter_secret', secret);
    }
    return secret;
  });
  ...
  const regenerateSecret = () => {
    ...
    localStorage.setItem('vaultproof_voter_secret', newSecret);
  ...
  const downloadReceipt = () => {
    ...
    a.href = url;
    a.download = `vaultproof_receipt_${receipt.checksum}.json`;
    a.click();
        ...
        <div className="glass-card mb-lg p-6 rounded-2xl">
          ...
          <div className="p-3.5 bg-black/40 rounded-xl border border-white/5 mb-md flex items-center justify-between flex-wrap gap-2">
            ...
            <button
              ...
              disabled={status === 'proving' || status === 'submitting'}
              className="text-xs text-muted hover:text-accent flex items-center gap-1 font-mono transition-colors"
